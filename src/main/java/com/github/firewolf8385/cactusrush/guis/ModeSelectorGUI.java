...
public class ModeSelectorGUI extends CustomGUI {
    ...
    public ModeSelectorGUI(CactusRush plugin, int teams, int teamSize) {
        ...
        setItem(23, mapSelector.build(), (p,a) -> new ArenaSelectorGUI(plugin, teams, teamSize).open(p));
    }

    private class ArenaSelectorGUI extends CustomGUI {
        public ArenaSelectorGUI(CactusRush plugin, int teams, int teamSize) {
            ...
            for(Arena arena : plugin.getArenaManager().getArenas()) {
                ...
                setItem(slot, builder.build(), (p,a) -> plugin.getGameManager().addToGame(p, teams, teamSize, arena.getId()));
                slot++;
            }
        }
    }
